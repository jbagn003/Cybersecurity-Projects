...
I wanted hands-on practice with the side of security that doesn't get the spotlight: provisioning users correctly the first time, designing access around least privilege instead of convenience, and actually catching it when that design drifts.

So I built **Bagnis Labs**, a fictional healthcare technology company, inside a real Google Workspace tenant, and ran it through a full identity governance cycle: design the access model, onboard and offboard users, simulate the kind of privilege creep that happens in every real org, catch it in a quarterly access review, and fix it. I also configured the email authentication stack (SPF/DKIM/DMARC) and reviewed OAuth app governance, since both show up constantly in real SOC and IAM-analyst job functions
